...
function RenderFunctionWithPointsOnly(f) {
  ...
  for (let x = MinX(); x <= MaxX(); x += xPrecision) {
    ...
    if (first) {
      ...
      Ctx.fillStyle = color;
      if (xPrecision < 0.5) Ctx.fillRect(XC(x), YC(y), 3, 3);
      else Ctx.fillRect(XC(x), YC(y), 5, 5);
    }
  }
  Ctx.stroke();
}
...
function resizeCanvas() {
  if (
    (window.innerWidth - 100 > 600 && window.innerWidth - 100 < 1200) ||
...
export {
  ...
  DrawAxes,
  RenderFunction,
  RenderFunctionFromList,
  RenderFunctionWithPointsOnly,
